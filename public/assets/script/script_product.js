...
function displayProductDetails(product) {
  ...
  const imageUrl =
    product.sach_bia_sach?.url_bia_chinh || APP_ENV.PLACEHOLDER_IMAGE;
  ...
  if (authorElement) {
    ...
    if (authorSpan) {
      if (
        product.tac_gias &&
        Array.isArray(product.tac_gias) &&
        product.tac_gias.length > 0
      ) {
        authorSpan.textContent = product.tac_gias
          .map((author) => author.ten_tac_gia)
          .join(", ");
      } else {
...
function displayNotFoundMessage() {
  const container =
    document.querySelector(".product-details-container") || document.body;
  container.innerHTML =
    '<div class="error-message"><h2>Không tìm thấy sản phẩm</h2><p>Sản phẩm bạn đang tìm kiếm không tồn tại hoặc đã bị xóa.</p></div>';
}
...
function fetchRelatedProducts(productId) {
  fetch(APP_ENV.MASTER_URL)
    ...
    .then((data) => {
      ...
      if (
        currentProduct.the_loais &&
        Array.isArray(currentProduct.the_loais) &&
        currentProduct.the_loais.length > 0
      ) {
        ...
        relatedProducts = products
          .filter(
            (product) =>
              ...
              product.the_loais.some(
                (cat) => cat.ma_the_loai === currentCategory
              )
          )
    ...
    .catch((error) => {
      ...
      if (container) {
        container.innerHTML =
          '<div class="error-message">Không thể tải sách liên quan</div>';
      }
    });
}
  ...
  container.innerHTML = products
    ...
    <div class="product-series">
      ...
      <div class="product-series-info">
        ...
        <p class="price">${
          product.gia_tien
            ? Number(product.gia_tien).toLocaleString("vi-VN") + "đ"
            : "Không có giá"
        }</p>
      </div>
    </div>
